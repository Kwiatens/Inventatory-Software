...
const vector<CatalogueProfile> kProfiles = {
  ...
  {"nexperia-discretes-v1","1","Nexperia","Diodes, BJTs and MOSFETs","",{"nexperia"},{"Orderable part number","Type number","MPN"},{"Type number"},{"Orderable part number"},{"Package"},{"Family"},{"Description"},{"Status"},{{{"VR","VCEO","VDS"},"voltage_rating","V",""},{{"IF","IC","ID"},"current_rating","A",""},{{"IFSM"},"surge_current","A",""},{{"VF"},"forward_voltage","V",""},{{"IR","Leakage current"},"leakage_current","A",""},{{"trr","Reverse recovery time"},"reverse_recovery_time","s",""},{{"Capacitance","Cd"},"capacitance","F",""},{{"VGS"},"gate_voltage","V",""},{{"RDS(on)"},"rds_on","Ohm",""},{{"VGS(th)","Threshold voltage"},"threshold_voltage","V",""},{{"hFE","DC current gain"},"dc_current_gain","",""},{{"fT","Transition frequency"},"transition_frequency","Hz",""},{{"Ptot"},"power","W",""},{{"Automotive"},"automotive_qualification","",""}},false},
...
const vector<ManufacturerSource>& manufacturerSources() { static const vector<ManufacturerSource> sources={
 ...
 {"tdk","TDK","TDK", "https://product.tdk.com/en/products/capacitor/ceramic/mlcc/index.html",{".csv",".xlsx"},{"MLCC"},DownloadMode::BrowserGuided,false,false,"tdk-mlcc-v1","Open Search by Characteristics; use a local CSV/XLSX only when TDK provides one for the selected result."},
 {"kemet-yageo","KEMET / Yageo","KEMET / Yageo", "https://www.kemet.com/en/us/capacitors/ceramic.html",{".csv",".xlsx"},{"Capacitors","MLCC"},DownloadMode::BrowserGuided,false,false,"kemet-yageo-mlcc-v1","Open Browse Ceramic. KEMET's public product pages do not currently expose a general catalogue export."},
 {"vishay","Vishay","Vishay", "https://www.vishay.com/en/resistors-fixed/current-sensing/",{".csv",".xlsx"},{"Current-sense resistors"},DownloadMode::BrowserGuided,false,false,"vishay-current-sense-v1","Use Export as CSV for an importable table. The Excel export is a series table; exact matching requires an orderable-MPN column."},
 ...
 {"ti","Texas Instruments","Texas Instruments", "https://www.ti.com/amplifiers",{".csv",".xlsx"},{"Amplifiers","MOSFETs","Timers"},DownloadMode::BrowserGuided,false,false,"ti-parametric-v1","Choose a product family and View all products. TI no longer has the former global parametric-search page."},
...
CatalogueImportStats importCatalogueTable(const filesystem::path& databasePath, const filesystem::path& path,
  ...
  const int aliasCol = findColumn(table.headers, profile->aliasColumns);
  ...
  const set<int> structuralColumns = {mpnCol, baseCol, aliasCol, packageCol, seriesCol, descriptionCol, statusCol};
  ...
  for (size_t index = 0; index < table.rows.size(); ++index) {
    ...
    if (!seriesOnly) addAlias(cell(row, aliasCol), "explicit");
...
CatalogueImportPreview CatalogueDatabase::previewFile(const filesystem::path& path, const CatalogueProfile* selected,
  ...
  const set<int> structural = {identityColumn, findColumn(table.headers, profile->basePartColumns), findColumn(table.headers, profile->aliasColumns),
