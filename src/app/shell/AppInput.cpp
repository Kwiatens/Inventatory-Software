    ...
    switch (kind) {
      ...
      case UiTargetKind::Cell:
        ...
        if (focused) {
          ...
          // Overlay the marker so the target keeps the element's original
          // geometry and therefore the same mouse hit box.
          element = ftxui::dbox({move(element), styledText(">", uiFocusColor(), uiSelectionBg())});
        }
        break;
