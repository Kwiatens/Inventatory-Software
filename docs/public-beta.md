...
For a new installation of the current prerelease, `v0.2.0-rc.6`, open Command
Prompt and paste this single command:

    curl.exe -fL "https://github.com/Kwiatens/Inventatory-Software/releases/download/v0.2.0-rc.6/Install-Inventatory.ps1" -o "%TEMP%\Install-Inventatory.ps1" && powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%TEMP%\Install-Inventatory.ps1" && del /q "%TEMP%\Install-Inventatory.ps1"
...
prerelease tags such as `v0.2.0-rc.6` are also accepted by the updater so
...
For Ubuntu 24.04 LTS, download the `v0.2.0-rc.6` Linux release assets into one
directory and run the checksum-verifying installer:

    mkdir -p "$HOME/.cache/inventatory-installer" && cd "$HOME/.cache/inventatory-installer" && curl -fLO "https://github.com/Kwiatens/Inventatory-Software/releases/download/v0.2.0-rc.6/Inventatory-linux-x64.tar.gz" && curl -fLO "https://github.com/Kwiatens/Inventatory-Software/releases/download/v0.2.0-rc.6/SHA256SUMS-linux.txt" && curl -fLO "https://github.com/Kwiatens/Inventatory-Software/releases/download/v0.2.0-rc.6/Install-Inventatory.sh" && chmod +x Install-Inventatory.sh && ./Install-Inventatory.sh
