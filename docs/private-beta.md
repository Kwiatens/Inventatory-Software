...
```cmd
mkdir "%TEMP%\Inventatory-install" && gh release download --repo Kwiatens/Inventatory-Software --pattern "Install-Inventatory.*" --dir "%TEMP%\Inventatory-install" --clobber && call "%TEMP%\Inventatory-install\Install-Inventatory.cmd"
```

The installer verifies the release ZIP against `SHA256SUMS.txt`, installs only for the current Windows user, preserves inventory data during updates, and asks before launching Inventatory. Run the same command later to update. The installed application performs a daily private-release check through the authenticated GitHub CLI session; it never sends inventory or Scan R1 data.
